...
async function startAnimation() {
  try {
    ...
    await writeTo(workEl, workText, 0, speed, false, 8);
...
async function writeTo(el, message, index, interval, mirrorToStyle, charsPerInterval){
  ...
  if (index < message.length) {
    ...
    if (endOfSentence.test(thisSlice)) thisInterval = interval * 60;
...
//
  ...
  function createWorkBox() {
    ...
    require('mouse-wheel')(workEl, async function(dx, dy) {
      ...
      let half = (workEl.scrollHeight - workEl.clientHeight) / 4;
